    ...
    <div class="main-content-inner">
        <div class="row">
            ...
            <div class="col-12 mt-5">
                <div class="card">
                    <div class="card-body">
                        <h4 class="header-title float-left">Les Abonnés</h4>
                        <!-- Filter Form -->
                                    ...
                                    @foreach ($users as $user)
                                        <tr>
                                            ...
                                            @if ($user->email_verified_at != null)
                                                <td>{{ $user->email }} <span class="badge badge-success">Vérifier</span>
                                                </td>
                                            @else
                                                <td>{{ $user->email }} <span class="badge badge-secondary">Non
                                                        vérifier</span></td>
                                            @endif
                                            <td>{{ $user->num }}</td>
                                            <td>{{ $user->city }}</td>
                                            <td>{{ $user->address }}</td>
                                            <td>
                                                @if (auth()->user()->can('subscriber.show'))
                                                    <a class="btn btn-info text-white"
                                                        href="{{ route('admin.subscribers.show', $user->id) }}">Details</a>
                                                @endif
                                                @if (auth()->user()->can('ban.create'))
                                                    <a href="javascript:void(0);" class="btn btn-warning text-white"
                                                        onclick="openBanModal({{ $user->id }}, '{{ $user->first_name }}')">Interdire</a>
                                                @endif
