  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Offre d'emploi supprimée avec succès.")),
      );
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Erreur lors de la suppression de l'offre.")),
      );
    }
  } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
          ...
          : _hasError
              ? const Center(child: Text("Erreur lors du chargement des offres."))
              : _userJobs.isEmpty
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                _buildInfoRow(Icons.category, 'Catégorie', job['category_name']),
                                _buildInfoRow(Icons.work, 'Type', job['job_type']),
                                ...
                                _buildInfoRow(Icons.calendar_today, 'Date limite',
                                    job['application_deadline']),
                                _buildInfoRow(Icons.info_outline, 'Statut', _getStatusLabel(status),
                                    iconColor: _getStatusColor(status),
                                    textColor: _getStatusColor(status)),
                                _buildInfoRow(Icons.note_add, 'Note', _getStatusNote(status, note)),
                                if (job['employer_type'] == 'entreprise') ...[
                                  ...
                                  if ((job['company_description'] ?? '').toString().isNotEmpty)
                                    Text('Description : ${job['company_description']}'),
